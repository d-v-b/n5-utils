...
def mvn():
    ...
    call(["mvn", "-Dmdep.outputFile={0}".format(cpfile),
          "-Dmdep.includeScope=runtime", "dependency:build-classpath"], shell=True)
    return 0
...
def make_exec(path, cmd):
    ...
    exec_path = Path(path) / 'n5-{0}.{1}'.format(cmd.lower(), ext)
    exec_path.parent.mkdir(parents=True, exist_ok=True) 
    jarpath = Path.home() / '.m2/repository/org/janelia/saalfeldlab/n5-utils/{0}/n5-utils-{0}.jar'.format(VERSION)
    # read the classpath file generated by mvn
    cp = Path(cpfile).read_text()
    if opsys == 'Windows':
      # get around stupid command prompt line limit by adding escaped newlines after each class
      cp = str.join(';^\n', cp.split(';'))    
    script = 'java -Xmx{0}g -XX:+UseConcMarkSweepGC -cp {1};{2} org.janelia.saalfeldlab.{3} {4}'.format(
        mem, jarpath, cp, cmd, argcapture)
